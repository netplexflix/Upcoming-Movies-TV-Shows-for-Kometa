...
# Known YML filename bases generated by UMTK and TSSK
_UMTK_YML_BASES = [
    'UMTK_TV_UPCOMING_SHOWS_OVERLAYS',
    'UMTK_TV_UPCOMING_SHOWS_COLLECTION',
    'UMTK_TV_NEW_SHOWS_OVERLAYS',
    'UMTK_TV_NEW_SHOWS_COLLECTION',
    'UMTK_MOVIES_UPCOMING_OVERLAYS',
    'UMTK_MOVIES_UPCOMING_COLLECTION',
]

_TSSK_YML_BASES = [
    'TSSK_TV_NEW_SEASON_OVERLAYS',
    'TSSK_TV_NEW_SEASON_COLLECTION',
    'TSSK_TV_NEW_SEASON_STARTED_OVERLAYS',
    'TSSK_TV_NEW_SEASON_STARTED_COLLECTION',
    'TSSK_TV_UPCOMING_EPISODE_OVERLAYS',
    'TSSK_TV_UPCOMING_EPISODE_COLLECTION',
    'TSSK_TV_UPCOMING_FINALE_OVERLAYS',
    'TSSK_TV_UPCOMING_FINALE_COLLECTION',
    'TSSK_TV_SEASON_FINALE_OVERLAYS',
    'TSSK_TV_SEASON_FINALE_COLLECTION',
    'TSSK_TV_FINAL_EPISODE_OVERLAYS',
    'TSSK_TV_FINAL_EPISODE_COLLECTION',
]

_ALL_YML_BASES = _UMTK_YML_BASES + _TSSK_YML_BASES


def cleanup_stale_ymls(kometa_folder, output_mode, instance_names=None, debug=False):
    """Remove stale YML files when switching between combined/split modes.

    In combined mode: delete any suffixed YML files (from a previous split run).
    In split mode: delete the non-suffixed versions of the same files.
    Also removes YML files for instances that no longer exist.
    """
    if not kometa_folder or not Path(kometa_folder).exists():
        return

    kometa_path = Path(kometa_folder)
    removed = []

    if output_mode == 'combined':
        # Remove any suffixed files: e.g. UMTK_TV_UPCOMING_SHOWS_OVERLAYS_Radarr4K.yml
        for base in _ALL_YML_BASES:
            for f in kometa_path.glob(f'{base}_*.yml'):
                # Make sure it's actually a suffixed variant, not a different file
                stem = f.stem
                if stem != base and stem.startswith(base + '_'):
                    f.unlink()
                    removed.append(f.name)
    elif output_mode == 'split':
        # Remove non-suffixed files
        for base in _ALL_YML_BASES:
            plain = kometa_path / f'{base}.yml'
            if plain.exists():
                plain.unlink()
                removed.append(plain.name)

        # Remove YML files for instances that no longer exist
        if instance_names is not None:
            valid_suffixes = {f'_{sanitize_instance_name(n)}' for n in instance_names}
            for base in _ALL_YML_BASES:
                for f in kometa_path.glob(f'{base}_*.yml'):
                    stem = f.stem
    ...
    if removed and debug:
        print(f"{BLUE}[DEBUG] Cleaned up stale YML files: {', '.join(removed)}{RESET}")